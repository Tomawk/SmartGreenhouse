...
#define SERVER_EP "coap://[fd00::1]:5683" //coap://[fd00::1]:5683
#define CONNECTION_TRY_INTERVAL 1
#define REGISTRATION_TRY_INTERVAL 1
#define SIMULATION_INTERVAL 1
#define SENSOR_TYPE "soilmoisture"

/* Log configuration */
#include "sys/log.h"
#define LOG_MODULE "soilmoisture"
#define LOG_LEVEL LOG_LEVEL_APP

#define INTERVAL_BETWEEN_SIMULATIONS 2
#define INTERVAL_BETWEEN_CONNECTION_TESTS 1

extern coap_resource_t res_irrigation;
extern coap_resource_t res_moisture;
...
PROCESS(soilmoisture_server, "SoilMoisture Server");
AUTOSTART_PROCESSES(&soilmoisture_server);
...
PROCESS_THREAD(soilmoisture_server, ev, data){
	PROCESS_BEGIN();

	static coap_endpoint_t server_ep;
    static coap_message_t request[1]; // This way the packet can be treated as pointer as usual

    leds_set(LEDS_NUM_TO_MASK(LEDS_RED)); //set led to red, irrigation is off at startup
	PROCESS_PAUSE();

	LOG_INFO("Starting SoilMoisture Server\n");
	coap_activate_resource(&res_irrigation, "soilmoisture/irrigation"); 
	coap_activate_resource(&res_moisture, "soilmoisture/moisture"); 
