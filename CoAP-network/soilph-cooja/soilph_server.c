...
#define SENSOR_TYPE "soilph"

/* Log configuration */
#include "sys/log.h"
#define LOG_MODULE "soilph"
...
PROCESS(soilph_server, "SoilPH Server");
AUTOSTART_PROCESSES(&soilph_server);
...
PROCESS_THREAD(soilph_server, ev, data){
	PROCESS_BEGIN();

	static coap_endpoint_t server_ep;
    static coap_message_t request[1]; // This way the packet can be treated as pointer as usual

    leds_set(LEDS_NUM_TO_MASK(LEDS_RED)); //set led to red, fertilizer is off at startup
	PROCESS_PAUSE();

	LOG_INFO("Starting SoilPH Server\n");
	coap_activate_resource(&res_fertilizer, "soilph/fertilizer"); 
	coap_activate_resource(&res_phsensor, "soilph/sensorph"); 
